...
char *parityCalculator(char *message);
...
char *parityCalculator(char *trueMessage)
{
    int messagelength = strlen(trueMessage);
...
char *formattedMessageSender(char *messageWithReceiver){
    ...
    char *encriptedbits = parityCalculator(message);
    ...
    finishedMessage[strlen(finishedMessage) + 1] = '\0';
...
int main()
{
    char *testmessage = "bu ne len mk> mahmut";
    char *finalMessage = formattedMessageSender(testmessage);
    printf("this is message %s",finalMessage);
}
